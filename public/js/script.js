...
function populateInvoiceComboboxes(relationId, invoiceId) {
  ...
  for (var i=0; i<invoices.length; i++) {
    ...
    if (!invoicesForRelationMap.hasOwnProperty(relation.public_id)) {
      invoicesForRelationMap[relation.public_id] = [];
    }

    invoicesForRelationMap[relation.public_id].push(invoice);
  ...
  $relationSelect.on('change', function(e) {
    ...
    for (var i=0; i<list.length; i++) {
      ...
      if (!relation) continue; // relation is deleted/archived
