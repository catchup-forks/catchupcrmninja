...
@section('head')
	@parent
		...
		@foreach ($invoice->relation->organisation->getFontFolders() as $font)
	...
	<div class="container">
        ...
        @else
            ...
            @if ($invoice->is_quote)            
                ...
                @endif
    		@elseif ($invoice->relation->organisation->isGatewayConfigured() && !$invoice->isPaid() && !$invoice->is_recurring)
                {!! Button::normal(trans('texts.download_pdf'))->withAttributes(['onclick' => 'onDownloadClick()'])->large() !!}&nbsp;&nbsp;
		...
		<script type="text/javascript">
			...
			invoice.is_pro = {{ $invoice->relation->organisation->isPro() ? 'true' : 'false' }};
		...
		@include('invoices.pdf', ['organisation' => $invoice->relation->organisation, 'viewPDF' => true])
