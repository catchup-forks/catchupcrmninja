...
@section('content')
    	...
    	<div class="col-md-4" id="col_1">
    		...
    		@if ($invoice->id || $data)
				<div class="form-group">
					<label for="relation" class="control-label col-lg-4 col-sm-4">{{ trans('texts.relation') }}</label>
					<div class="col-lg-8 col-sm-8">
                        <h4><div data-bind="text: getRelationDisplayName(ko.toJS(relation()))"></div></h4>
                        
                        @if($invoice->relation->canView())
                            @if ($invoice->relation->canEdit())
                                <a id="editRelationLink" class="pointer" data-bind="click: $root.showRelationForm">{{ trans('texts.edit_relation') }}</a> |
                            @endif
			...
			<div class="form-group" style="margin-bottom: 8px">
				<div class="col-lg-8 col-sm-8 col-lg-offset-4 col-sm-offset-4">
					<a id="createRelationLink" class="pointer" data-bind="click: $root.showRelationForm, html: $root.relationLinkText"></a>
                    <span data-bind="visible: $root.invoice().relation().public_id() > 0" style="display:none">|
                        <a data-bind="attr: {href: '{{ url('/relations') }}/' + $root.invoice().relation().public_id()}" target="_blank">{{ trans('texts.view_relation') }}</a>
                    </span>
				</div>
			</div>
				...
				<div style="display:none" class="form-group" data-bind="visible: contacts().length > 0 &amp;&amp; (contacts()[0].email() || contacts()[0].first_name()), foreach: contacts">
					<div class="col-lg-8 col-lg-offset-4 col-sm-offset-4">
						<label class="checkbox" data-bind="attr: {for: $index() + '_check'}" onclick="refreshPDF(true)">
                            <input type="hidden" value="0" data-bind="attr: {name: 'relation[contacts][' + $index() + '][send_invoice]'}">
							<input type="checkbox" value="1" data-bind="checked: send_invoice, attr: {id: $index() + '_check', name: 'relation[contacts][' + $index() + '][send_invoice]'}">
				...
				<td class="hide-border" colspan="{{ 2 + ($organisation->showCustomField('custom_invoice_item_label1') ? 1 : 0) + ($organisation->showCustomField('custom_invoice_item_label2') ? 1 : 0) }}" rowspan="6" style="vertical-align:top">
                    ...
                    <div role="tabpanel">
                      ...
                      <ul class="nav nav-tabs" role="tablist" style="border: none">
                        <li role="presentation" class="active"><a href="#notes" aria-controls="notes" role="tab" data-toggle="tab">{{ trans('texts.note_to_relation') }}</a></li>
        ...
        <div class="row" data-bind="with: relation" onkeypress="relationModalEnterClick(event)">
            <div style="margin-left:0px;margin-right:0px" data-bind="css: {'col-md-6': $root.showMore}">
                ...
                @if (Auth::user()->isPro())
                    @if ($organisation->custom_relation_label1)
                        {!! Former::text('relation[custom_value1]')
                            ->label($organisation->custom_relation_label1)
                    ...
                    @if ($organisation->custom_relation_label2)
                        {!! Former::text('relation[custom_value2]')
                            ->label($organisation->custom_relation_label2)
                ...
                <span data-bind="visible: $root.showMore">
                    ...
                    {!! Former::text('relation[housenumber]')
                            ->label(trans('texts.housenumber'))
                            ->data_bind("value: housenumber, valueUpdate: 'afterkeydown'") !!}
         ...
         <div class="modal-footer" style="margin-top: 0px; padding-top:0px;">
            ...
            <button id="relationDoneButton" type="button" class="btn btn-primary" data-bind="click: $root.relationFormComplete">{{ trans('texts.done') }}</button>
         </div>
	...
	$(function() {
		...
		@if ($invoice->id || count($relations) == 0)
    ...
    function onFormSubmit(event) {
        ...
        var relationCurrencyId = model.invoice().relation().currency_id() || {{ $organisation->getCurrencyId() }};
    ...
    function setInvoiceNumber(relation) {
        @if ($invoice->id || !$organisation->hasRelationNumberPattern($invoice))
            return;
        @endif
