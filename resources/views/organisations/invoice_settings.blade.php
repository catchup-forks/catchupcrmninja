...
@section('content')	
	@parent
    @include('organisations.nav', ['selected' => ORGANISATION_INVOICE_SETTINGS, 'advanced' => true])

    {!! Former::open()->rules(['iframe_url' => 'url'])->addClass('warn-on-exit') !!}
    {{ Former::populate($organisation) }}
    {{ Former::populateField('auto_convert_quote', intval($organisation->auto_convert_quote)) }}
    {{ Former::populateField('custom_invoice_taxes1', intval($organisation->custom_invoice_taxes1)) }}
    {{ Former::populateField('custom_invoice_taxes2', intval($organisation->custom_invoice_taxes2)) }}
    {{ Former::populateField('share_counter', intval($organisation->share_counter)) }}
                ...
                <div role="tabpanel" class="tab-pane active" id="invoiceNumber">
                    <div class="panel-body">
                        {!! Former::inline_radios('invoice_number_type')
                                ...
                                ])->check($organisation->invoice_number_pattern ? 'pattern' : 'prefix') !!}
                        ...
                        {!! Former::text('invoice_number_counter')
                                ...
                                ->help(trans('texts.invoice_number_help') . ' ' . 
                                    trans('texts.next_invoice_number', ['number' => $organisation->previewNextInvoiceNumber()])) !!}
                ...
                <div role="tabpanel" class="tab-pane" id="quoteNumber">
                    <div class="panel-body">
                        {!! Former::inline_radios('quote_number_type')
                                ...
                                ])->check($organisation->quote_number_pattern ? 'pattern' : 'prefix') !!}
                        ...
                        {!! Former::text('quote_number_counter')
                                ...
                                ->help(trans('texts.quote_number_help') . ' ' . 
                                    trans('texts.next_quote_number', ['number' => $organisation->previewNextInvoiceNumber(ENTITY_QUOTE)])) !!}
	...
	<script type="text/javascript">
  	...
  	function setQuoteNumberEnabled() {
			...
			$('#quote_number_counter').val(disabled ? '' : '{!! $organisation->quote_number_counter !!}');
		}
