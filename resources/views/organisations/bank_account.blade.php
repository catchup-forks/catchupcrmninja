...
@section('head')
    ...
    <style type="text/css">
        table.organisations-table > thead > tr > th.header {
...
@section('content')
    ...
    @include('organisations.nav', ['selected' => ORGANISATION_BANKS])
    ...
    <div class="panel-body">
        ...
        <div class="col-lg-12 col-sm-12" data-bind="visible: page() == 'setup'" style="display:none">
            <table class="table organisations-table" style="width:100%">
                <thead>
                    <tr>
    ...
    function loadTransactions(data) {
        ...
        for (var i=0; i<data.length; i++) {
            var row = data[i];
            var organisation = new BankAccountModel(row);
            if (row.transactions) {
                var transactions = organisation.transactions();
                ...
                organisation.transactions.valueHasMutated();
            }
            model.bank_accounts.push(organisation);
        }
    }
        ...
        self.vendor.pretty = ko.computed({
            ...
            write: function (value) {
                ...
                for (var i=0; i<model.bank_accounts().length; i++) {
                    var organisation = model.bank_accounts()[i];
                    var transactions = organisation.transactions();
    ...
    var ViewModel = function() {
        ...
        self.includedTransactions = ko.computed(function() {
            ...
            for (var i=0; i<self.bank_accounts().length; i++) {
                ...
                var transactions = ko.utils.arrayFilter(organisation.transactions(), function(transaction) {
