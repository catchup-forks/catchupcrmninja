...
@section('content')
    ...
    @include('organisations.nav', ['selected' => ORGANISATION_PAYMENTS])
    ...
    @if ($OrganisationGateway)
        {!! Former::populateField('gateway_id', $OrganisationGateway->gateway_id) !!}
        {!! Former::populateField('payment_type_id', $paymentTypeId) !!}
        {!! Former::populateField('recommendedGateway_id', $OrganisationGateway->gateway_id) !!}
        {!! Former::populateField('show_address', intval($OrganisationGateway->show_address)) !!}
        {!! Former::populateField('update_address', intval($OrganisationGateway->update_address)) !!}
        {!! Former::populateField('publishable_key', $OrganisationGateway->getPublishableStripeKey() ? str_repeat('*', strlen($OrganisationGateway->getPublishableStripeKey())) : '') !!}

        @if ($config)
            @foreach ($OrganisationGateway->fields as $field => $junk)
                @if (in_array($field, $hiddenFields))
                    {{-- do nothing --}}
                @elseif (isset($config->$field))
                    {{ Former::populateField($OrganisationGateway->gateway_id.'_'.$field, $config->$field) }}
                @endif
            @endforeach
        @endif
    @else
    ...
    @foreach ($gateways as $gateway)
        ...
        <div id="gateway_{{ $gateway->id }}_div" class='gateway-fields' style="display: none">
            ...
            @foreach ($gateway->fields as $field => $details)
                ...
                @if ($details && !$OrganisationGateway)
