...
@section('content')
	...
	<div class="row">
		<div class="col-md-3">
            ...
            @if ($vendor->address1)
                {{ $vendor->address1 }}<br/>
            @endif
            @if ($vendor->housenumber)
                {{ $vendor->housenumber }}<br/>
            @endif
            ...
            @if ($vendor->organisation->custom_vendor_label1 && $vendor->custom_value1)
                {{ $vendor->organisation->custom_vendor_label1 . ': ' . $vendor->custom_value1 }}<br/>
            @endif
            @if ($vendor->organisation->custom_vendor_label2 && $vendor->custom_value2)
                {{ $vendor->organisation->custom_vendor_label2 . ': ' . $vendor->custom_value2 }}<br/>
            @endif
    ...
    @if ($vendor->hasAddress())
        function initialize() {
            ...
            var address = "{{ "{$vendor->address1} {$vendor->housenumber} {$vendor->city} {$vendor->state} {$vendor->postal_code} " . ($vendor->country ? $vendor->country->name : '') }}";
