...
@section('body')
          ...
          <div class="navbar-form navbar-right">
            ...
            <div class="btn-group user-dropdown">
              <button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown">
                <div id="myOrganisationButton" class="ellipsis nav-organisation-name"
                     style="max-width:{{ Utils::isPro() && ! Utils::isTrial() ? '130' : '100' }}px;">
            ...
            <li class="dropdown">
              @if (Utils::isAdmin())
                ...
                <ul class="dropdown-menu">
                  ...
                  @endforeach
                  <li><a
                      href="{{ url('settings/' . ORGANISATION_INVOICE_SETTINGS) }}">{!! uctrans('texts.advanced_settings') . Utils::getProLabel(ORGANISATION_ADVANCED_SETTINGS) !!}</a>
                  </li>
                </ul>
              @else
  ...
  @if (!Auth::check() || !Auth::user()->registered)
    <div class="modal fade" id="signUpModal" tabindex="-1" role="dialog" aria-labelledby="signUpModalLabel"
          ...
          <div style="background-color: #fff; padding-right:20px" id="signUpDiv" onkeyup="validateSignUp()"
            ...
            <div class="modal-footer" id="signUpFooter" style="margin-top: 0px">
              <button type="button" class="btn btn-default" id="closeSignUpButton"
                      data-dismiss="modal">{{ trans('texts.close') }} <i class="glyphicon glyphicon-remove-circle"></i>
              </button>
      ...
      @if (Auth::check() && (!Auth::user()->isPro() || Auth::user()->isTrial()))
        <div class="modal fade" id="proPlanModal" tabindex="-1" role="dialog" aria-labelledby="proPlanModalLabel"
             aria-hidden="true">
              ...
              <div class="row">
                ...
                <div class="col-md-7 left-side">
                  <center>
                    ...
                    <img class="img-responsive price" alt="Only $50 Per Year"
                         src="{{ asset('images/pro_plan/price.png') }}"/>
                    @if (Auth::user()->isEligibleForTrial())
                      <a class="button"
                         href="{{ URL::to('start_trial') }}">{{ trans('texts.trial_call_to_action') }}</a>
                    @else
                      <a class="button" href="#"
                         onclick="submitProPlan()">{{ trans('texts.pro_plan_call_to_action') }}</a>
                    @endif
                  </center>
                </div>
                <div class="col-md-5">
